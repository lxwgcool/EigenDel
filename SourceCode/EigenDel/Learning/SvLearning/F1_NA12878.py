...
def _plotFMeasures(fstepsize=.05, stepsize=0.001):
    ...
    p = sc.arange(0.0, 0.66, stepsize)[1:]
    #print(p)
    #for f in sc.arange(0, 1., fstepsize)[1:]:
    for f in sc.arange(0.001, 0.6, fstepsize)[1:]:
        ...
        pl.annotate(r"$f1=%.2f$" % f, xy=( xs[-130], ys[-10]), 
                    xytext=( xs[-250], ys[-210]), 
                    size="small", color="gray")
        #print("xy    : ", xs[0], ys[0])
        #print("xytext: ", xs[-50] - .2, ys[-10])
...
def plotPrecisionRecallDiagram(title="title", points=None, labels=None, loc="center right"):
    ...
    pl.xlabel("Accuracy", fontsize = 12)
    pl.ylabel("Sensitivity", fontsize = 12)    
    _plotFMeasures()
    ...
    if points:                    
        ...
        if labels:
            ...
            pl.legend(scps, labels, loc=(0.77, 0.01), scatterpoints=1, 
    ...
    pl.axis([-0.02, 0.66, 0.01, 0.78])  # xmin, xmax, ymin, ymax


if __name__ == '__main__':
     plotPrecisionRecallDiagram("(A) F1 Score of NA12878",
                                points=[(0.58, 0.51), (0.59, 0.43), (0.6377, 0.44), (0.0019,0.74), (0.11, 0.07), (0.25, 0.37)], 
                                labels=["EigenDel", "Delly", "Lumpy", "Pindel", "CNVnator", "GASVpro"])
